...
export default async function DualFlowInvitePage({ searchParams }: InvitePageProps) {
      ...
      <div className="container max-w-md py-16">
        <div className="rounded-lg border border-destructive bg-destructive/10 p-6">
          ...
          <p className="text-muted-foreground">
            No se encontró un token de invitación válido. Asegúrate de usar el enlace completo que
            te enviaron.
          </p>
        </div>
      </div>
  ...
  if (!user) {
    redirect(`/login?returnUrl=/dual-flow/invite?token=${token}`);
  }
    ...
    return (
      <div className="container max-w-md py-16">
        <div className="rounded-lg border border-destructive bg-destructive/10 p-6">
          ...
          <p className="text-sm text-muted-foreground">Posibles razones:</p>
